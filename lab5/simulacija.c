...
//dodaje dretvu na kraj reda
void dodaj_na_kraj(int index){
	P[brojac_dretvi] = malloc( sizeof(dretva) );
	P[brojac_dretvi]->id = nove[index][1];
	P[brojac_dretvi]->p = nove[index][2];
	P[brojac_dretvi]->prio = nove[index][3];
	P[brojac_dretvi]->rasp = nove[index][4];
}
...
void  ubaci_nove(){
	while( t == nove[do_kud_sam_stigao][0] ){ //dolazi nam nove dretva
		printf( "  %d -- nova dretva id=%d, p=%d, prio=%d\n", t, nove[do_kud_sam_stigao][1],
						nove[do_kud_sam_stigao][2], nove[do_kud_sam_stigao][3] );
